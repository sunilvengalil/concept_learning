...
At the most 10 sample images are annotated by a human after each epoch of training
\end{abstract}
...
The approach proposed in this paper needs very less amount of manual annotation ( samples in case of MNIST dataset)  and less computing resources.
We demonstrate our claim by building a classification model for MNIST dataset using all traning images, but using the labels partially.
We use only limited number of labelled samples from the training set during each epoch of training.
The proposed approach augments a  variational autoencoder with a classification layer the loss component of which is tuned by manually annotated samples at regular training intervals.
...
In our experiments, we used $k=10$ which is same as the number of classes in MNIST dataset.
...
Each sample in the cluster is assigned with the  same label as the cluster center. Each sample is also given a confidence based on its distance from cluster center and  a manually assigned confidence in the range of [0,1].
...
All the samples from the un-identifiable clusters at the lowest level ( we limited our experiment to 2 levels) are manually annotated and given an overall confidence of 1.
A confidence of 1 is justified here since the training samples are annotated and not the cluster centers.
...
We trained the network for a total of 5 epochs in three different ways.
\begin{enumerate}
    ...
    \item Use only the classification loss  for all 5 epochs by sampling $10000$ labelled samples randomly from the training set
\end{enumerate}
...
We choose $z_{dim}=10$ to run the  semi-supervised model, exeriment 2 mentioned above,  with both reconstruction and classification loss.
...
The proposed approach is demonstrated by classifying MNIST images by annotating only 10 images, corresponding to each cluster center,  after each epoch.
...
It is also observed that the reconstruction accuracy of the generative model improves as a result of performing an auxilary task of classification
