...
\begin{abstract}
    With the introduction of deep neural networks, many tasks like image classification, object detection and semantic segmentation can be solved if a lot of annotated data is available.
Since these models are complex with millions of parameters in practice, huge amounts of labelled samples are required to train such models without over-fitting.
For many practical applications, plenty of unlabelled samples are available but getting these samples labelled is a tedious manual task.
In this work, we introduce a novel mechanism to label the samples iteratively starting with a completely unlabelled dataset by clustering the samples in a latent space learned using Variational Autoencoder.
We demonstrate the method using MNIST images and obtain a classification accuracy of 92% by labelling only 50 samples.
...
if the cluster center for a cluster does not correspond to any valid digit image, that cluster is again clustered into $k$ clusters and a further attempt is made to label the cluster centers of 2nd level cluster.
