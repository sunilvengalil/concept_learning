...
Since MNIST images are centered, in order to achieve translation invariance during training, each image was translated by $t$ pixels in one of the four direction - up, down, left and right.
For each image one of the direction is chosen randomly from the set \{up,down,left,right\} with equal probability for each direction.
Also the number of pixels to translate $t$ are obtained by chosing an interger from the set \{0,1,2,3,4\} again with equal probablity for each element.
