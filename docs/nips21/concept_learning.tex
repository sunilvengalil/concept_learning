...
However, one of the key tenets for all these works is to learn a complex parameterized unknown function using back propagation.
When trained with huge amount of data for long enough durations and with appropriate ragularization techniques, these networks are capable of learning functions that generalize well.
However few of the challange with these  models are
\begin{enumerate}
...
In this work, our key focus is on how to force a deep neural network to learn semantically meaningful features and representations. Our work can also be used as an active learning mechanism using deep neural networks. The core of the idea is to incorporate human feedback into the training processes.
