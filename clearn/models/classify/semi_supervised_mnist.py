...
class SemiSupervisedClassifierMnist(VAE):
    ...
    def train(self, train_val_data_iterator):
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for batch in range(start_batch_id, self.num_batches_train):
                ...
                sample_weights = np.ones([64,1]).reshape(-1)
            ...
            print(f"Epoch: {epoch}, Nll_loss: {nll_loss} KLD:{kl_loss}  Supervised loss:{supervised_loss} ")
    ...
    def encode_and_get_features(self, images: np.ndarray, gradient_layer = -1):
        ...
        print(f"Gradient computing for layer {gradient_layer}")
        if gradient_layer >= 0:
            output_tensor = self.y_pred
            layer_tensor = hidden_features[gradient_layer]
            gradient = tf.gradients(output_tensor, layer_tensor)
