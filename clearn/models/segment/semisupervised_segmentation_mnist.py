...
class SemiSupervisedSegmenterMnist(VAE):
    ...
    def _decoder(self, z, reuse=False):
        return fully_deconv_n_layer(self, z, self.exp_config.num_units, self.exp_config.dao.image_shape[2], 1, reuse)
    ...
    def evaluate(self,
        ...
        if self.exp_config.return_latent_vector:
            ...
            for i, sigma_col_name in enumerate(sigma_col_names):
                encoded_df[sigma_col_name] = np.reshape(sigma, [sigma.shape[0], -1])[:,i]

            for i, z_col_name in enumerate(z_col_names):
                encoded_df[z_col_name] = np.reshape(z, [z.shape[0], -1])[:,i]
