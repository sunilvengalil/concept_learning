...
#   Gaussian Encoder
    ...
    def build_model(self):
        ...
        self.labels = tf.placeholder(tf.float32, [bs, label_dim], name='manual_label')
        ...
        #supervised loss
        #if sample has a label
        self.y_pred = linear(self.z,10)
        supervised_loss= tf.losses.softmax_cross_entropy(onehot_labels=self.labels,
                                                         logits=self.y_pred,
        ...
        self.loss = -ELBO + self.supervised_weight *  self.supervised_loss
    ...
    def train(self,train_val_data_iterator):
        ...
        self.num_batches_train = train_val_data_iterator.get_num_samples("train") // self.batch_size
        self.num_eval_batches =  train_val_data_iterator.get_num_samples("val") // self.batch_size
            ...
            for idx in range(start_batch_id, self.num_batches_train):
                ...
                _, summary_str, loss, nll_loss, kl_loss = self.sess.run(
                    ...
                    feed_dict={self.inputs: batch_images,
                               ...
                               self.is_manual_annotated:labels[:,10],
    ...
    def evaluate(self, epoch, step, counter, val_data_iterator ):
        ...
        for _idx in range(start_eval_batch, self.num_eval_batches):
            ...
            _batch_eval_labels = np.concatenate([batch_eval_labels, integer_label], axis=1)
            columns = [str(i) for i in range(10)]
            columns.append("label")
            pd.DataFrame(_batch_eval_labels,
                         columns=columns)\
            ...
            reconstructed_image, summary = self.sess.run([self.out, self.merged_summary_op],
                                                         feed_dict={self.inputs: batch_eval_images,
                                                                    self.labels:batch_eval_labels[:,:10],
                                                                    self.is_manual_annotated:np.zeros(batch_eval_labels.shape[0]),
                                                                    self.standard_normal: batch_z})
    ...
    def get_encoder_weights_bias(self):
        ...
        w_3, b_3, w_4, b_4 = self.sess.run([default_graph.get_tensor_by_name(name_w_3),
                                            default_graph.get_tensor_by_name(name_b_3),
                                            default_graph.get_tensor_by_name(name_w_4),
                                            default_graph.get_tensor_by_name(name_b_4)])
        return {"fc3": [w_3, b_3],"fc4": [w_4, b_4]}
